...
- также вручную создал файл для переноса данных с БД /db/tools/save_dump.py, акиуализировал данные для подключения;
...
Что сделано:
- отредактировал докер-композ: создал папку на HDD 
```
sudo mkdir -p /data/easysochi/postgres_honey
sudo chown -R 999:999 /data/easysochi/postgres_honey
```
и смонтировал в нее том БД
...
volumes:
  honey_bot_postgres_data:
    ...
    driver_opts:
      type: none
      o: bind
      device: /data/easysochi/postgres_honey

```
- скопировал дамп, поднял базу данных
- подключился к СУБД, протестил - данные актуальны
- при создании контейнера с ботом создается новый юзер в Dockerfile и все файлы передаются ему. Меньше уязвимости.
- отредактировал порты в .env
- запустил приложение
- запушил код на ГитХаб

Проблеме:
- сервер закрыт снаружи, чтобы подключиться к БД через СУБД используется метод SSH
- внутри общение по сети
- логгер в браузере пока не доступен, нужно монтировать nginx
- телеграм люто виснет, очень долго отвечает, настроил таймаут при создании приложения, но иногда все равно не успевает отвечать 

2026-02-24

Что сделано:
- в разделе RAID создал папки для хранения медиа и логов, раздал права пользователям
```
sudo mkdir -p /data/easysochi/media_honey
sudo mkdir -p /data/easysochi/logs_honey
sudo chown -R 1000:1000 /data/easysochi/media_honey
sudo chown -R 1000:1000 /data/easysochi/logs_honey
```
- отредактировал docker compose
...
[] - написать сценарий быстрой проверки после обновления - основной функционал
...
[] - добавилась папка для хранения медиа на диске, необходимо перенести хранение и отдачу статической информации (приветственное фото, правила, пдфы) с диска
...
[] - приглашение на дегустацию надо персонализировать (обращение сделать???)
[] - в ЛК надо показывать также заказы в статусе Клиент проинформирован с кнопкой Куплено
[] - переписать логику хранения изображений продуктов на хост в папку Медиа (не хранить больше в ТГ)
